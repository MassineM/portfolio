<template>
  <div
    ...
    @mouseover="mouseOnSide"
    @mouseout="mouseNotSide"
  >
    <nav>
      ...
      <div id="shadowCurr" class="currshadow"></div>
      <ul>
        <li
          v-for="item of nav"
          v-html="item.svg"
          ...
          @click="mouseClick"
          :class="{
            currIcon: item.key == currTab,
            selIcon: item.text == selTab && !(item.key == currTab),
            icon: true,
          }"
        >
          <!-- <img :id="item.key" :src="require(`${iconsPath}${item.url}`)" /> -->
        </li>
      </ul>
    </nav>
  </div>
...
export default {
  ...
  methods: {
    mouseIsMoving(e) {
      ...
      if (!this.onBanner && !this.onTab && yPercent < 0.94) {
        document.getElementById("shadow").style.opacity = (0.3 - xPercent) * 3;
