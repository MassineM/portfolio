...
export default {
  ...
  mounted: function () {
    ...
    document.addEventListener("mousewheel", this.onScroll.bind(this), { passive: false });
  ...
  methods: {
    ...
    selectCurrent(item) {
      ...
      for (const li of this.nav)
        if (li.key == item.key) {
          ...
          this.pointer = i;
          document.getElementById(this.arrHref[i]).scrollIntoView({behavior: "smooth", block: "start", inline: "nearest"});
          i++;
        } else {li.selected = false; i++;}
    },
    get_hovered_text() {
      for (const item of this.nav) if (item.hovered) return item.text;
    },
    hoverMyItem(item) {
      for (const li of this.nav) {
        if (li.key == item.key) {
    ...
    onScroll(e){
      ...
      this.isScrolling = 1;
      setTimeout(()=>{this.isScrolling = 0}, 500);
      console.log(e);
      if(e.deltaY > 0)
        this.pointer = (this.pointer + 1) % this.arrHref.length;
      else
      { 
        this.pointer = (this.pointer - 1) % this.arrHref.length;
        if(this.pointer < 0) this.pointer += this.arrHref.length;
      }
      console.log(this.pointer);
      // document.getElementById(this.arrHref[this.pointer]).scrollIntoView();
